...
Let's take two prime numbers, {\fontfamily{pcr}\selectfont 13} and {\fontfamily{pcr}\selectfont 7} (and don't reveal them to anyone).

Let's multiply them together: we get {\fontfamily{pcr}\selectfont n=91}.

We choose a public key: {\fontfamily{pcr}\selectfont 5}.

We use the \textit{Extended Euclidean Algorithm} with inputs {\fontfamily{pcr}\selectfont 5}, {\fontfamily{pcr}\selectfont 7} and {\fontfamily{pcr}\selectfont 13}, and we get the private key: {\fontfamily{pcr}\selectfont 29}.

So {\fontfamily{pcr}\selectfont n=91}, public key {\fontfamily{pcr}\selectfont 5}, private key {\fontfamily{pcr}\selectfont 29}.

Let Alice and Bob know that {\fontfamily{pcr}\selectfont n=91}, Alice's public key is {\fontfamily{pcr}\selectfont 5}, and her private key is {\fontfamily{pcr}\selectfont 29}.

Bob wants to send a message to Alice: the message is {\fontfamily{pcr}\selectfont 3}.

Then multiply {\fontfamily{pcr}\selectfont 3} by itself {\fontfamily{pcr}\selectfont 5} (public key) times, modulo {\fontfamily{pcr}\selectfont n=91}: {\fontfamily{pcr}\selectfont 3*3*3*3*3=243}, {\fontfamily{pcr}\selectfont mod(243,91)=61}.

The public message is {\fontfamily{pcr}\selectfont 61}.

By doing the opposite we can decode the message.

{\fontfamily{pcr}\selectfont a=3; at=3;}

{\fontfamily{pcr}\selectfont for i=1:5-1}

{\fontfamily{pcr}\selectfont  \ at=mod(at*a,91)}

{\fontfamily{pcr}\selectfont end}

{\fontfamily{pcr}\selectfont Output=61}

{\fontfamily{pcr}\selectfont a=61; at=61;}

{\fontfamily{pcr}\selectfont for i=1:29-1}

{\fontfamily{pcr}\selectfont  \ at=mod(at*a,91)}

{\fontfamily{pcr}\selectfont end}

{\fontfamily{pcr}\selectfont Output=3.}

We have decoded the message!

\begin{itemize}
	\item Alice and Bob know {\fontfamily{pcr}\selectfont n=91}.
	\item Everyone knows {\fontfamily{pcr}\selectfont 5}.
	\item Only Alice knows {\fontfamily{pcr}\selectfont 29}.
	\item Knowing {\fontfamily{pcr}\selectfont 5} (public key) is not sufficient to decrypt the message.
	\item Can Bob trace Alice's private key? That is, is the information {\fontfamily{pcr}\selectfont 91} sufficient to compromise security?
...
\fg{0.7}{c5d0d8444570c81ebcd5ac910f0741e7-HutyWMPv53-image}
...
\begin{enumerate}
	...
	\item The 64-character string is then put into a Bitcoin (or other) blockchain transaction using the {\fontfamily{pcr}\selectfont OP\_RETURN} \textbf{field}, it's a \textit{free} field presented in every Bitcoin transaction: free, but limited in size.
...
{\fontfamily{pcr}\selectfont [08-CBDM.pptx]}
...
\textbf{Merkle tree} is a digest of digests, i.e. a tree, a compact representation of a set of hashes of Bitcoin transactions, amongst the proposed and selected ones. The root represents all the hashes in the tree and any change anywhere would affect the root.
\begin{itemize}
...
\fg{0.7}{0ab69fc7ee15e9b5efb04bb8d39299ab-2lIIJXa4gj-image}
